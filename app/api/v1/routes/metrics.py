...
from app.api.schemas.metrics import (
    ...
    ModuleHealthItemSchema, FlakyTestItemSchema, StableTestItemSchema, TrendItemSchema, MTTRSummarySchema,
...
from app.api.dependencies import (
    ...
    get_module_health_use_case, get_flaky_tests_use_case, get_stable_tests_use_case,
...
# ==========================================================
#  GET /metrics/stable-tests
# ==========================================================

@router.get(
    "/stable-tests",
    summary="List most stable tests based on pass rate",
    response_model=List[StableTestItemSchema],
    responses={
        200: {"description": "Lista de testes estaveis ordenados por total de execucoes"},
        400: {"model": ErrorResponseSchema, "description": "Erro de validacao"},
    },
)
def get_stable_tests(
    project: str = Query(..., description="Nome do projeto (obrigatorio)"),
    environment: str = Query(..., description="Ambiente do projeto (obrigatorio)"),
    filters: MetricsFilter = Depends(get_metrics_filters),
    use_case: GetStableTestsUseCase = Depends(get_stable_tests_use_case)
):
    """
    Identifies tests that consistently pass across multiple runs.
    Returns the test name, total executions, and pass rate.
    """
    result = use_case.execute(
        project.lower(), environment.lower(), filters.start_dt, filters.end_dt, filters.last_runs)
